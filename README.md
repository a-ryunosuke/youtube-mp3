...
動画投稿サイトの音声を抽出し、入力したMP3タグ（タイトル・アーティスト名・コメント）を埋め込んだ状態でダウンロードできるアプリケーション

### 開発の動機
既存の変換サイトにおける「操作感の悪さ」「タグ書き込み非搭載」「広告の多さ」「変換時間の長さ」といった不満を解消するために制作。※趣味のDJ活動で必要な楽曲を、個人の範囲内で効率的に取得することを目的としています。
...
- **ユーザー認証**: 認証することで変換履歴を参照可能。
- **変換履歴**: 過去にダウンロードした動画の履歴を保存。履歴一覧から対象のログ削除にも対応。
- **バックエンド**:
