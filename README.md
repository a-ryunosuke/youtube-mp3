...
動画投稿サイトの音声を抽出し、入力したMP3タグ（タイトル・アーティスト名・コメント）を埋め込んだ状態でダウンロードするアプリケーション

### 開発の動機
既存の変換サイトにおける「操作感の悪さ」「タグ書き込み非搭載」「広告の多さ」「変換時間の長さ」といった不満を解消する為に作成しました。
※趣味のDJ活動で必要な楽曲を、個人遊戯の範囲内で取得するツールです。
...
- **MP3変換 & タグ挿入**: それぞれのフォームに入力された情報を元に、ダウンロード後のファイルのタグ情報に値が挿入されます。
- **バリデート機能**: React Hook Form + Zod によるバリデート機能。

- **ユーザー認証**: ログイン・ログアウト機能。ログインすることで、変換履歴を参照可能になります。
- **変換履歴**: 過去にダウンロードした動画の履歴を保存。また対象ログの削除に対応。※ログイン時のみ利用可能です。
- **バックエンド**:
...
- **ライトモード/ダークモード**: デバイスの設定に応じて自動で切り替わります。
</p>
...
作成初期vanillaCssで作成していましたが、学習コストの観点からtailwindCssに変更しました。
