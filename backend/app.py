...
def progress_hook(d):
    if d['status'] == 'downloading':
        # ダウンロード進行中はパーセントが更新されるたびに呼ばれる
        # ただし、最初に1回だけ "downloading" を送るように制御しても良い
...
# index.htmlを読み込み表示(frontはreactなので今回は不要)
# @app.route('/')
# def index():
...
def download():
    ...
    # 元のファイル名をMP3タグとして使用、拡張子は消える(見えなくなる)
    set_metadata(final_output_file or file_name, file_name, artist_name, comment)
